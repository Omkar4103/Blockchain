...
//the smart contract is migrated/deployed to a local blockchain using ganache
//testing done using chai and mocha frameworks provided by truffle
//testing happens locally without the use of actual ether 
...
contract("Contest", function(accounts){
   ...
   before(async function() {
      contestInstance = await Contest.deployed();
      contestantId = 2;
   });
...
it("it initializes the cotestants with the correct values", function(){
	return Contest.deployed().then(function(instance){
		...
		assert.equal(contestant[2], 0, "contains the correct vote counts");
